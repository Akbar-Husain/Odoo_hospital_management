...
#     invoice_state = fields.Selection(related = 'invoice_id.state', string = 'Invoice', states = {'cancel': [('readonly', True)], 'done': [('readonly', True)]})
    ...
    amount_total = fields.Float(string = 'Charges', store = True, readonly = True, track_visibility = 'always') #compute = '_amount_all',
...
#             self.other_services_line = line
    ...
    @api.model
    def button_cancel(self, values):
        self.state = 'cancel'

    @api.model
    def button_done_foc(self, values):
        self.state = 'done'

    @api.model
    def button_accepted_xray(self, values):
...
# #             self.sudo().invoice_id = invoice_id
        self.sudo().state = 'in_progress'

    @api.model
    def button_done(self, values):
        self.state = 'done'
