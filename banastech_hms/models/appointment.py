...
#         return self.env.ref("banastech_hms.reason_routine_follow")
    ...
    # On change of department_id,Only those docs visible which added in that department
    @api.onchange('department_id')
    def _onchange_department_id(self):
        if self.department_id:
            return {'domain': {'doctor_id': [('id', 'in', self.department_id.doctor_ids.ids)]}}
        else:
            return {'domain': {'doctor_id': []}}


    # On change of doctor_id,Only those departments visible which added in that doctor
    @api.onchange('doctor_id')
    def _onchange_doctor_id(self):
        if self.doctor_id:
            return {'domain': {'department_id': [('id', 'in', self.doctor_id.department_ids.ids)]}}
        else:
...
#     #Diseases
    ...
    purpose_id = fields.Many2one('banas.hms.appointment.purpose', string='Purpose', help="Appointment Purpose", states={'cancel': [('readonly', True)], 'done': [('readonly', True)], 'confirm': [('readonly', True)], 'waiting': [('readonly', True)]})
...
#     state_id = fields.Many2one(related="patient_id.state_id", string="State")
    ...
    def create_invoice(self):
        ...
        invoice = inv_obj.create({
            ...
            'l10n_in_gst_treatment': 'consumer',  # l10n_in module should be installed for this field
            'ref': self.name,
        ...
        invoice.action_post() # invoice gose posted on create_invoice()
...
class ChiefComplains(models.Model):
    ...
    _description = 'HMS Chief Complains'
