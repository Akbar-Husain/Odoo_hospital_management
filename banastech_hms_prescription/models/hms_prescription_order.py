...
#     invoice_id = fields.Many2one('account.invoice', ondelete="restrict", string='Invoice')
    appointment = fields.Many2one('banastech.hms.appointment', ondelete="restrict", string='Appointment', domain="[('patient_id', '=', patient_id)]")
    ...
    def action_sale_order(self):
        for ip_req in self:
            order = self.env['sale.order'].create({
                'partner_id' : ip_req.patient_id.id,
                'date_order' : fields.Datetime.now(),
